...
public class WhighamMutation extends ConfigOperator<GRModel> implements GRMutation {
	 ...
	 * 
	 */
	public WhighamMutation(final RandomNumberGenerator rng, int labels, int attributes) {
		this((GRModel) null);
		this.rng = rng;
		grower.setRNG(rng);
                this.n_labels = labels;
                this.n_attributes = attributes;
                grower.setNAttributes(n_attributes);
                grower.setNLabels(n_labels);
	}
	...
	public WhighamMutation(final GRModel model) {
		super(model);
		grower = new GrowInitialiser(model, this.n_labels, this.n_attributes);
	}
