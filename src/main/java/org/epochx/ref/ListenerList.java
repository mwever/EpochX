...
public class ListenerList<T extends Listener> extends AbstractList<T> {
	 ...
	 */
	public ListenerList() {
		this.data = new ArrayList<MixedReference<T>>();
	...
	protected void expunge() {
		...
		while ((ref = (MixedReference<? extends Listener>) this.referenceQueue.poll()) != null) {
	...
	public void add(final int index, final T element, final boolean strong) {
		final MixedReference<T> next = new MixedReference<T>(element, strong, this.referenceQueue);
	...
	public T remove(final int index) {
		final MixedReference<T> removed = this.data.remove(index);
		return removed.get();
	}
	...
	public T set(final int index, final T element, final boolean strong) {
		final MixedReference<T> next = new MixedReference<T>(element, strong, this.referenceQueue);
		final MixedReference<T> prev = this.data.set(index, next);
